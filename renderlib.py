...
def rendertask(task):
	...
	if task.outfile.endswith('.ts'):
		cmd = 'cd {0} && ffmpeg -ar 48000 -ac 2 -f s16le -i /dev/zero -f image2 -i .frames/%04d.png -pix_fmt yuv420p -c:v mpeg2video -qscale 0 -f mpegts -aspect 16:9 -shortest "{1}"'.format(task.workdir, task.outfile)
	else:

...
# URL to Schedule-XML
scheduleUrl = 'https://pretalx.com/fossgis2026/schedule/export/schedule.xml'
...
def introFrames(params):
	...
	for i in range(0, frames):
		yield (
			...
			('text3', 'style', 'opacity', "%.4f" % easeOutCubic(i, 0, 1, frames))
		)
...
def pauseFrames(params):
	...
	for i in range(0, frames):
		yield (
			...
			('text3', 'style', 'opacity', "%.4f" % (1-easeOutCubic(i, 0, 1, frames)))
		)
...
def tasks(queue, args, idlist, skiplist):
	...
	for event in events(scheduleUrl):
		if event['room'] not in ('HS1 (ZHG 011)', 'HS2 (ZHG 010)', 'HS3 (ZHG 009)', 'HS4 (ZHG 008)', 'Posterausstellung'):
		...
		if (event['id'] in idlist or not idlist) and not 'intro' in skiplist:
			...
			#elif event['id'] == 57449:
			#	event['personnames'] = 'Thomas Bartoschek'
			#if event['id'] == 57948:
